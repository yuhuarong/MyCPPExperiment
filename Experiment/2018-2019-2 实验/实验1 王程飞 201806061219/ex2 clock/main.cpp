...
int main() {
	...
	*/
	...
	for(int i=0;i<4;i++){
		cout<<"到达： ";arriveTime[i].toString();cout<<", 离开： ";leaveTime[i].toString();cout<<" 共超出 "<<(double)(leaveTime[i].diff(arriveTime[i]))<<" h 收费 "<<(double)(leaveTime[i].diff(arriveTime[i]))*4<<" 元"<<endl;
	}
