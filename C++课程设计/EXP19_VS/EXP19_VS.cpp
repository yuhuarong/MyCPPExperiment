...
int main() {
	...
	while (1) {
		...
		switch (code) {
		case '0':
			openFileForRead(file, path);
			break;
		case '1':
			openFileForWrite(file, path);
			break;
		case '2':
			addContract(file, path, mUI, mContract);
			break;
		case '3':
			deleteContract(file, path, mContract, mUI);
			break;
		case '4':
			updateContract(file, path, mContract, mUI);
			break;
		case '5':
			readContract(file, path, mContract, mUI);
			break;
		case '6':
			exit(file);
			break;
